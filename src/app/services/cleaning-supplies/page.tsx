...
export const metadata = {
  title: "Cleaning & Housekeeping Supplies | Quality Products & Services",
  description: "Bulk supply of cleaning chemicals, tissue products, dispensers, and housekeeping tools in Hyderabad.",
};

export default async function CleaningSuppliesPage() {
  const bgHero = await getPexelsImage("commercial cleaning supplies", "landscape");
      ...
      <section className="relative bg-[var(--color-brand-navy)] py-20 overflow-hidden flex items-center min-h-[40vh]">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10 w-full text-center flex flex-col items-center">
          ...
          <p className="text-[var(--color-text-on-dark-muted)] text-lg max-w-2xl mx-auto">
            Everything you need for pristine facility management.
          </p>
        </div>
      </section>

      {/* Corporate Framing Section */}
      <section className="py-16 bg-[var(--color-surface-grey)]">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <SectionHeading 
            title="Supply for Offices, Hotels & Facilities" 
            subtitle="We cater to the bulk supply needs of corporate offices, hospitality businesses, and large-scale facility management companies across Hyderabad."
          />
          <Button variant="whatsapp" className="mt-8 shadow-lg text-lg px-8 py-4" asChild>
            <a href="https://wa.me/919993454442?text=Hi%2C%20I%27d%20like%20to%20enquire%20about%20bulk%20cleaning%20%26%20housekeeping%20supplies.">
            ...
            {subCategories.map((cat, i) => (
              <div key={i} className="flex flex-col h-full">
                ...
                <Button variant="whatsapp" className="w-full mt-4 rounded-t-none" asChild>
                  <a href={`https://wa.me/919993454442?text=Hi%2C%20I%27d%20like%20to%20enquire%20about%20bulk%20cleaning%20%26%20housekeeping%20supplies%20(${encodeURIComponent(cat.title)}).`}>
                    Enquire on WhatsApp
                  </a>
                </Button>
              </div>
